...
def gen_sample(tparams, f_init, f_next, x, options, trng=None,
               k=1, maxlen=100, stochastic=True, argmax=False, send_alpha=False):
    ...
    #For planning
    #print options
    next_commit = numpy.ones((1, options['plan_step'])).astype('float32')
    next_action_plan = numpy.ones((x.shape[0], 1, options['plan_step'])).astype('float32')

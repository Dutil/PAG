...
def compute_accuracy(tparams, f_init, f_next, options, dataset, gen_sample):
    import accuracy

    all_pred = []
    all_true = []
    for xs, ys in dataset:
        res = accuracy.translate(tparams, f_init, f_next, options, xs, gen_sample, k=1, normalize=False)
        for pred, y in zip(res, ys):
    ...
    digit_acc = 0.
    tot_digits = 0
    for pred, y in zip(all_pred, all_true):
        y = [i for i in y if i != 0]
        pred = [i for i in pred if i != 0]
        tot_digits += len(y)

        len_ = min(len(y), len(pred))
        abs_acc += sorted(pred) == sorted(y)
        digit_acc += sum([y[i] == pred[i] for i in xrange(len_)])
        #digit_acc += len(set(pred).intersection(set(y))) / float(len(y))

    return abs_acc / len(all_true), digit_acc / tot_digits
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=True, verboseFreq=None):
    ...
    for x, y in iterator:
        ...
        x, x_mask, y, y_mask, n_x = prepare_data(x, y,
                                                 ...
                                                 n_words=options['n_words'], maxlen_trg=10,
                                                 maxlen=100)
...
def train(
    ...
    train = GraphIterator(source=datasets[0],
                         source_dict=dictionaries[0],
                         batch_size=batch_size,
                         sort_size=sort_size,
                         shuffle_per_epoch=shuffle_dataset)

    valid = GraphIterator(source=valid_datasets[0],
                         source_dict=dictionaries[0],
    ...
    if doing_planning:
        ...
        commit_penalty = ((c_lb * ((1./model_options['plan_step'] - probs)**2).sum(axis=-1)*y_mask).sum(axis=0))
        #commit_penalty = ((c_lb*((0.5 - probs)**2).sum(axis=-1)*y_mask).sum(axis=0))
    ...
    for epoch in xrange(max_epochs):
        ...
        for x, y in train:
            ...
            if numpy.mod(uidx, validFreq) == 0:
                ...
                abs_acc, digit_acc = compute_accuracy(tparams, f_init, f_next, model_options, valid, gen_sample)
                print "The absolute accuracy: ", abs_acc, ". The per digit accuracy: ", digit_acc

...
from core_d2qgen.commons import global_rng
from core_d2qgen.utils import sharedX
...
def print_params(tparams):
    ...
    for k, v in tparams.iteritems():
        ...
        print "pname: ", k, " shp: ", shp
        all_shps += numpy.prod(shp)

    print "Total number of params, ", all_shps
...
def init_tparams(params):
    ...
    for kk, pp in params.iteritems():
        tparams[kk] = sharedX(params[kk], name=kk)
    return tparams
...
def surround(str_, token="**"):
    str_ = str_.encode('utf-8').decode('ascii', 'ignore')
    return "{0}{1}{0}".format(str(token), str(str_))

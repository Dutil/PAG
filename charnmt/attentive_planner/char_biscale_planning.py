...
def init_params(options):
    ...
    # decoder
    params = get_layer('planning')[0](options, params,
                                             prefix='decoder',
...
def build_model(tparams, options):
    ...
    [char_h, word_h, bound_c, bound_w, ctxs, alphas, probs, samples, commit_origin, probs_origin, action_plans, temp], updates = \
            get_layer('planning')[1](tparams, yemb, options,
                                            prefix='decoder',
                                            mask=y_mask,
                                            context=ctx,
                                            context_mask=x_mask,
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    [next_state_char, next_state_word, next_bound_char, next_bound_word, next_ctx, next_alpha, probs, next_commit,
     commit_origin, probs_origin, next_action_plan, temp], updates = \
            get_layer('planning')[1](tparams, yemb, options,
                                            ...
                                            init_bound_char=init_bound_char,
                                            init_bound_word=init_bound_word,
                                     init_commit=init_commit,init_action_plan=init_action_plan,
                                     )
    ...
    inps = [y, ctx, init_state_char, init_state_word, init_bound_char, init_bound_word, init_commit, init_action_plan]
    #inps = [y, ctx, init_state_char, init_state_word, init_bound_char, init_bound_word, init_sample, init_action_plan]
    outs = [next_probs, next_sample, next_state_char, next_state_word, next_bound_char, next_bound_word, next_commit, next_action_plan, next_alpha]
    f_next = theano.function(inps, outs, name='f_next', profile=profile, updates=updates)
    print 'Done'
...
def gen_sample(tparams, f_init, f_next, x, options, trng=None,
    ...
    next_commit = numpy.zeros((1, options['kwargs']['plan_step'])).astype('float32')
    next_commit[0] = 1
    init_commit = next_commit
    ...
    for ii in xrange(maxlen):
        ...
        inps = [next_w, ctx, next_state_char, next_state_word, next_bound_char, \
                next_bound_word, next_commit, next_action_plan]
        ret = f_next(*inps)
        ...
        #For planning
        next_commit, next_action_plan, next_alpha = ret[6], ret[7], ret[8]
        ...
        if stochastic:
            ...
            #init_commit = np.roll(init_commit, axis=1)
